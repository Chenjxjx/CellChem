...
def _save_config_file(model_checkpoints_folder):
    if not os.path.exists(model_checkpoints_folder):
        ...
        shutil.copy(config_file, os.path.join(model_checkpoints_folder, os.path.basename(config_file)))


class Molpretrain(object):
    ...
    def _step(self, model, xis, xjs, n_iter):
        ...
        ris, zis = model(
            xis.x, xis.batch, 
            xis.edge_index, xis.edge_attr,
            xis.get('chem_desc', None),
        )  # [N,C]

        # get the representations and the projections
        rjs, zjs = model(
            xjs.x, xjs.batch, 
            xjs.edge_index, xjs.edge_attr,
            xjs.get('chem_desc', None),
        )  # [N,C]
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            ...
            model = self._load_pre_trained_weights(model)
        elif self.config['model_type'] == 'gt':
            from models.graphtransformer_molclr import GraphTransformer
...
def main():
    ...
    Mol = Molpretrain(dataset, config)
    Mol.train()
