...
hyperparameter_defaults = dict(
    ...
    dataset_name="clue",
    do_train=True,
    load_model="../../perturb_pretrain/save/dev_clue-May09-14-15",
    load_mol_model="../../mol_gt_pretrain/ckpt/May08_16-21-26/checkpoints",
...
model_file = model_dir / "model_e88.pt"
...
(
    ...
    all_counts, celltypes_labels, batch_ids, cond_ids, smiles_data=smiles, test_size=0.1, shuffle=True
)
...
if config.load_model is not None:
    try:
        model_scGPT.load_state_dict(torch.load(model_file))
...
graphtransformer = _load_pre_trained_weights(graphtransformer).to(device)
model =  CellChem(model_scGPT,graphtransformer).to(device)
...
def train(model: nn.Module, loader: DataLoader) -> None:
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            loss =nt_xent_criterion(cmap_embedding, mol_embedding)
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            logger.info(
                ...
                f"loss {cur_loss:5.2f} | " if config.GEPC else ""
            )
            total_loss = 0
            start_time = time.time()
