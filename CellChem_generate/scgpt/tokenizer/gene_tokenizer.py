...
import pandas as pd
import torch
from .simple_vocab import SimpleVocab
...
class GeneVocab(SimpleVocab):
    ...
    def __init__(
        self,
        gene_list_or_vocab: Union[List[str], SimpleVocab, List[str], Dict[str, int]],
    ...
    def _build_vocab_from_iterator(
        ...
        if specials:
            if special_first:
                ordered = specials + ordered
            else:
                ordered = ordered + specials
        return ordered
