...
mol_encoder = _load_pre_trained_weights(mol_encoder)
decoder = Decoder(atom_dim, hid_dim, n_layers, n_heads, pf_dim, DecoderLayer, SelfAttention,dropout, device)
model = Predictor(mol_encoder,encoder, decoder, device).to(device)
...
def _validate(model, valid_loader):
    ...
    with torch.no_grad():
        ...
        valid_loss = 0.0
        counter = 0
        for bn, (CRISPR,Cmap,Protein,Mol,label,Protein_len, Mol_len,idx) in enumerate(valid_loader):
...
config = yaml.load(open("CellChem_add_test.yaml", "r"), Loader=yaml.FullLoader)  
print(config)
from dataset.Dataset_fusion import MoleculeDatasetWrapper
dataset,_ = MoleculeDatasetWrapper(config['batch_size'], **config['dataset']).get_dataset()
...
for i in range(1,6):
    print(i)
    path = './Model_CellChem_random_save/'+str(i)+'_fold_model.pth'  ####(eg:CellChem_add_random_save,CellChem_add_scaffold_save)
...
df.to_csv('./test_result/CellChem_add_test_random.csv')  #(eg:CellChem_add_test_random.csv,CellChem_add_test_scaffold.csv)
